    ...
    <el-dialog
      ...
      >
        ...
        <el-form-item prop="roleCode">
          <template #label>
                  <span>
                     <el-tooltip content="控制器中定义的角色标识，如：@PreAuthorize(`@ss.hasRole('admin')`)" placement="top">
    ...
    const methods = {
      ...
      init: async (id) => {
        ...
        await menuApi.list({}).then((response) => {
          // 删除掉hasChildren不然不显示
          state.menuTree = treeDataTranslate(response.data)
        })

        // 获取部门树
        await deptAi.list({}).then((response) => {
          state.deptTree = treeDataTranslate(response.data)
        })
        ...
        // 处理赋值
        const menuCheckedKeys = state.dataForm.menuIds;
        const deptCheckedKeys = state.dataForm.deptIds;
      ...
      onSubmit: () => {
        dataFormRef.value.validate((valid) => {
          if (valid) {
            // 设置菜单ids和部门ids
            state.dataForm.menuIds = methods.getMenuAllCheckedKeys();
            state.dataForm.deptIds = methods.getDeptAllCheckedKeys();
            // 下面就是调用ajax
            roleApi
      ...
      handleCheckedTreeExpand: (value, type) => {
        if (type === "menu") {
          let treeList = state.menuTree;
          for (let i = 0; i < treeList.length; i++) {
            menuRef.value.store.nodesMap[treeList[i].id].expanded = value;
          }
        } else if (type === "dept") {
          let treeList = state.deptTree;
          for (let i = 0; i < treeList.length; i++) {
      ...
      handleCheckedTreeNodeAll: (value, type) => {
        if (type === "menu") {
          menuRef.value.setCheckedNodes(value ? state.menuTree : []);
        } else if (type === "dept") {
          deptRef.value.setCheckedNodes(value ? state.deptTree : []);
        }
      },
      ...
      handleCheckedTreeConnect: (value, type) => {
        if (type === "menu") {
          state.dataForm.menuCheckStrictly = !!value;
        } else if (type === "dept") {
          state.dataForm.deptCheckStrictly = !!value;
        }
      },
